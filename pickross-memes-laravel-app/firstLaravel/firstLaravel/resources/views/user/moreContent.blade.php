    ...
    <div class="row sharebar " style="margin-top:10px; ">
         <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6" > 
             ...
             <!-- <input type="hidden" id="likeValue{{$a->id}}" value="{{$a->like}}"> -->
              <a id="likebttn_{{$a->id}}" onclick="like({{$a->id}})" style="float: left; margin-left: 5px;" href="javascript:void(0);"><span id="likebttn_img_{{$a->id}}" class="glyphicon glyphicon-thumbs-up" style="color:#19887e; font-size:20px; "></span></a>    
             
             @else
              {!! Form::hidden('likeValue',$a->like,['id'=>'likeValue'.($a->id)]) !!}
          <!--    <input type="hidden" id="likeValue{{$a->id}}" value="{{$a->like}}"> -->
             <a id="likebttn_{{$a->id}}" onclick="like({{$a->id}})" style="float: left; margin-left: 5px;" href="javascript:void(0);"><span  id="likebttn_img_{{$a->id}}" class="glyphicon glyphicon-thumbs-up" style="color:#999999; font-size:20px; "></span></a>
 
             
             @endif
             
            @if($a->comment==1) 
            <a  href="javascript:void(0);"  onclick="comment({{$a->id}})" style="margin-left: 5px;"><span id="commentbttn_img_{{$a->id}}" class="glyphicon glyphicon-comment" style="color:#19887e; font-size:20px;"></span> </a>
            @else
             <a  href="javascript:void(0);"  onclick="comment({{$a->id}})" style="margin-left: 5px;"><span id="commentbttn_img_{{$a->id}}" class="glyphicon glyphicon-comment" style="color:#999999; font-size:20px;"></span> </a>
         
            @endif
            
             @if($a->report==1)
              {!! Form::hidden('reportValue',$a->report,['id'=>'reportValue'.($a->id)]) !!}
            <a class="tool" id="reportbttn_{{$a->id}}" onclick="report({{$a->id}})" style=" margin-left: 5px; margin-right:-10px;" data-toggle="tooltip" data-placement="right" title="Report"><span id="reportbttn_img_{{$a->id}}" class="glyphicon glyphicon-flag" style="font-size:20px; color:#999999;"></span></a>
             @else
                {!! Form::hidden('reportValue',$a->report,['id'=>'reportValue'.($a->id)]) !!}
            <a class="tool" id="reportbttn_{{$a->id}}" onclick="report({{$a->id}})" style=" margin-left: 5px; margin-right:-10px;" data-toggle="tooltip" data-placement="right" title="Report"><span id="reportbttn_img_{{$a->id}}" class="glyphicon glyphicon-flag" style="font-size:20px; color:#999999;"></span></a>   
             @endif
